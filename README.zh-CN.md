...
3. 在“应用程序”中打开 LidGuard。由于当前安装包使用临时签名且未经 Apple 公证，macOS 可能显示 **“未打开 ‘LidGuard’”**，并提供“完成 / 移到废纸篓”两个按钮。此时点击 **“完成”**，不要移到废纸篓。
4. 打开 **“系统设置 → 隐私与安全性”**，向下找到“安全性”中关于 LidGuard 被阻止的提示，点击 **“仍要打开”**。
5. 验证身份，并在新的确认框中点击 **“打开”**。这个绕过操作只需为当前版本执行一次。
6. 进入 LidGuard，点击 **“安装 Helper”**，并完成一次管理员授权。

> [!IMPORTANT]
> 只有从本仓库 Release 下载并确认来源可信后，才应使用“仍要打开”。如果“仍要打开”没有出现，请先再次尝试从“应用程序”打开 LidGuard，然后立即返回“隐私与安全性”。

安装 Helper 时会同时安装 LaunchDaemon 和 `lidguard` CLI。完成首次授权后，日常切换“合盖运行 / 正常休眠”不再需要管理员密码。
覆盖安装临时签名的新版本后，如果一级面板提示 Helper 需要修复，直接点击 **“一键修复并恢复连接”** 并输入一次管理员密码；修复完成后界面会自动刷新。
